...
inline std::pair<int, int> find_square(const Painting& p, Tile t, int s) {
    for (int r = t.r1; r <= (t.r2 - s); ++r) {
        for (int c = t.c1; c <= (t.c2 - s); ++c) {
...
Cmd get_best_square(const Painting& p, Tile t) {
    int max_size = std::min(abs(t.c2 - t.c1), abs(t.r2 - t.r1));
...
Solution solve(const Painting& p) {
    ...
    while (tiles.size() > 0) {
        ...
        solution.commands.push_back(split.second);
    }
